...
const ThemeItem: React.FC<{ label: string; themeName: string }> = (props) => {
  ...
  async function setTheme() {
    theme.setTheme(props.themeName);
  ...
  return (
    <input
      ...
      checked={theme.theme === props.themeName}
      onClick={() => setTheme()}
      readOnly
    />
  );
...
function ThemeControllerDropdown() {
  return (
    <div className="dropdown dropdown-end">
      ...
      <ul
        tabIndex={0}
        className="dropdown-content z-[1] p-2 shadow-2xl bg-base-300 rounded-box w-52 overflow-y-auto max-h-[300px]"
      >
        <ThemeList />
      </ul>
    </div>
